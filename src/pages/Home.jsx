...
const CAROUSEL_ITEMS = [
    { label: '1:1 Class' },
    { label: '1:Many Classes' },
    { label: 'Video Program' },
    { label: 'Music Playlist' },
    { label: 'Find Your Connection' },
    { label: 'Badges & Levels' },
    { label: 'Referral Code' },
    { label: 'Dance Challenges' },
    { label: 'Scrolling Feed' },
...
const Home = () => {
    ...
    const eventCards = t('home.eventCards');
    const studioCards = t('home.studioCards');
                            ...
                            <div className="w-56 sm:w-64 md:w-72 animate-float">
                                <div className="bg-gray-900 rounded-[3rem] p-3 shadow-2xl border border-white/10 animate-pulse-glow">
                                    ...
                                    <div className="bg-gradient-to-b from-brand-gradientStart to-brand-dark rounded-[2.3rem] overflow-hidden aspect-[9/19] flex flex-col items-center justify-center border border-white/5">
                                        ...
                                        <p className="text-white/30 text-xs font-medium text-center px-4">{t('home.app.phonePlaceholder')}</p>
                                        <p className="text-white/20 text-[10px] mt-1">Upload app screenshot</p>
                                    </div>
                            ...
                            >
                                ...
                                <div className="h-48 relative overflow-hidden">
                                    <video autoPlay loop muted playsInline className="event-video">
                                        <source src={DANCE_VIDEOS[idx % DANCE_VIDEOS.length]} type="video/mp4" />
                                    </video>
                        ...
                        {Array.isArray(studioCards) && studioCards.map((studio, idx) => (
                            <div key={idx} className={`bg-white/5 rounded-2xl border border-white/10 overflow-hidden hover:border-brand-gold/30 transition-all duration-300 card-hover ${studiosVisible ? `animate-fade-in-up delay-${(idx + 1) * 200}` : 'opacity-0'}`}>
                                <div className="h-40 overflow-hidden">
                                    <img src={STUDIO_IMAGES[idx % STUDIO_IMAGES.length]} alt={studio.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" loading="lazy" />
                                </div>
                                <div className="p-5 sm:p-6">
